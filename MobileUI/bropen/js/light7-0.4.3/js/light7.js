...
+function ($) {
  ...
  var showTab = function (tab, tabLink, force) {
    ...
    if (tabLink) tabLink = $.look(tabLink);
    else {
      // Search by id
      if (typeof tab === 'string') tabLink = $.look('.tab-link[href="' + tab + '"]');
      else tabLink = $.look('.tab-link[href="#' + newTab.attr('id') + '"]');
      // Search by data-tab
      if (!tabLink || tabLink && tabLink.length === 0) {
        $.look('[data-tab]').each(function () {
    ...
    if (oldTab && oldTab.length > 0) {
      ...
      if (oldTabId) oldTabLink = $.look('.tab-link[href="#' + oldTabId + '"]');
      // Search by data-tab
...
+function ($) {
  "use strict";
    ...
    $.modal = function (params) { // params中新增参数：modalCloseByOutside，当点击空白处时，是否隐藏modal
        params = params || {};
        var modalHTML = '';
        if (defaults.modalTemplate) {
            ...
            modalHTML = '<div class="modal ' + noButtons + (params.modalCloseByOutside ? '' : ' noModalCloseByOutside') + '"><div class="modal-inner">' + (titleHTML + textHTML + afterTextHTML) + '</div><div class="modal-buttons ' + verticalButtons + '">' + buttonsHTML + '</div></div>';
        }
    ...
    $.showPreloader = function (title, modalCloseByOutside) {
        return $.modal({
            title: title || defaults.modalPreloaderTitle,
            text: '<div class="preloader"></div>',
            modalCloseByOutside : modalCloseByOutside == true
        });
    };
    ...
    function handleClicks(e) {
        ...
        if (clicked.hasClass('modal-overlay')) {
            if ($('.modal.modal-in:not(.noModalCloseByOutside)').length > 0 && defaults.modalCloseByOutside)
                $.closeModal('.modal.modal-in');
...
+ function($) {
    ...
    $.initPullToRefresh = function(pageContainer) {
        ...
        // 防止重复渲染
        if ( eventsTarget.data('initPullToRefresh') ) return;
        eventsTarget.data('initPullToRefresh', true)
...
+ function($) {
    ...
    $.initInfiniteScroll = function(pageContainer) {
        ...
        // 防止重复渲染
        if ( infiniteContent.data('initInfiniteScroll') ) return;
        infiniteContent.data('initInfiniteScroll', true);
...
+function ($) {
  ...
  $(document).on("click", ".open-panel", function(e) {
    var panel = $(e.target).data2(panel);
    $.openPanel(panel);
  });
  ...
  ************   Swipe panels   ************
  ...
  $.initSwipePanels = function () {
       ...
       * 禁用侧边栏
       */
      $.disableSwipePanels = function() {
          $(document).off($.touchEvents.start, handleTouchStart);
          $(document).off($.touchEvents.move, handleTouchMove);
          $(document).off($.touchEvents.end, handleTouchEnd);
      }
  };
...
+function ($) {
  ...
  Router.prototype.init = function() {
    ...
    if(!state) {
      ...
      this.replaceState(location.href, id, null);
  ...
  Router.prototype.loadPage = function(url, noAnimation, replace, reload, isPushState, leftToRight) { // 新增参数，是否记录历史、方向
    ...
    var url = param.url, noAnimation = param.noAnimation, replace = param.replace,
        reload = param.reload, isPushState = param.isPushState, leftToRight = param.leftToRight, isChangeState = param.isChangeState,
        isNewPage = url != location.href;

    var page = $('.page[data-url="' + encodeURIComponent(url) + '"]');
    if ( param.reload ) {
      if ( !page.hasClass('page-current') ) this.removePage(page, this, true);
    } else if ( page.length && page.data('fail') !== true ) { // 如果页面存在，不需要再ajax加载页面
      _loadPage.call(this, page, null, true);
      return;
    }

    this.getPage(url, _loadPage, param.pageid);

    /**
     * 加载成功回调事件
     * @param page
     * @param extra
     * @param isExist   页面是否已经存在于html中
     * @private
     */
    function _loadPage(page, extra, isExist) {
      var currentPage = this.getCurrentPage();

      var pageid = currentPage[0].id;
      if ( !replace && isChangeState != false ) {
        var action = "pushBack";
        if (replace) action = "replaceBack";
        if (reload && !isNewPage) action = "reloadBack";
        this[action]({
            url: location.href,
            pageid: "#" + pageid,
      ...
      var self = this;
      if ( !reload && isChangeState != false ) {
        //remove all forward page
        var forward = JSON.parse(this.state.getItem("forward") || "[]");
        for(var i=0;i<forward.length;i++) {
          $(forward[i].pageid).each(function() {
            var $page = $(this);
            if($page.data("page-remote") && !$page.hasClass('page-current') && $page[0] != page[0]) { // 解决bug：打开首页，点其他页面，再点首页，回退2次，再点其他页面，页面背景出现空白，中间出现刷新层，一直刷新
              removePage($page, self);
            }
          });
        }
      ...
      if(duplicatePage[0] !== page[0]) {
        this.removePage(duplicatePage, null, true);
      }
      ...
      if(extra && extra.length) {
        ...
        extra.each(function(i, el) {
          var id = el.id ? el.id : 'pageExt-' + (+new Date());
      ...
      if ( isPushState !== false )
        this[replace || (reload && !isNewPage) ? "replaceState" : "pushState"](url, id, leftToRight);

      if ( isExist !== true ) page.trigger('loaded', [page[0].id, page, extra]); // 添加loaded完成事件
      this.forwardStack  = [];  //clear forward stack

      // 动画前添加事件
      if ( currentPage && currentPage.length )
        currentPage.trigger('beforePageSwitch', [currentPage[0].id, currentPage]);
      // replace页面，动画后把之前的页面删掉
      if ( replace ) {
        currentPage.one('pageSwitched', function() {
          removePage(currentPage, self);
        });
      }
      if ( leftToRight )
        this.animatePages(page, currentPage, leftToRight, noAnimation);
      else
        this.animatePages(currentPage, page, leftToRight, noAnimation);
    }
  }

  Router.prototype.removePage = removePage;
  /**
   * 删除指定页面
   * @param $page       所要删除的页面
   * @param self        当前对象，如果为空时，取this
   * @param forceDelete 强制删除
   */
  function removePage(selector, self, forceDelete) {
    ...
    for ( var i=0; i<selector.length; i++ ) {
      ...
      if ( page instanceof HTMLElement ) {
        var $page = $(page);
        // 页面为当前页面 或 页面设置了缓存，没有强制删除时，不删除此页面
        if ( ( !forceDelete && $page.data('cache') ) || $page.hasClass('page-current') ) return;
        var pageExtra = self.extras[$page[0].id];
        pageExtra && pageExtra.remove();
        self.extras[$page[0].id] = undefined;
        $page.trigger('beforePageRemove', [$page[0].id, $page]); // 新增删除前事件
        $page.remove();
        $.router.dispatch("pageRemoved"); // 新增删除成功事件
      }
    }
  }

  //load new page and replace current page inhistory
  Router.prototype.replacePage = function(url, noAnimation, reload, isPushState, leftToRight) {
    return this.loadPage(url, noAnimation, true, reload, isPushState, leftToRight);
  }
  ...
  Router.prototype.animatePages = function (leftPage, rightPage, leftToRight, noTransition) {
    ...
    if(noTransition) {
      if (!leftToRight) {
        ...
        leftPage.trigger('pageSwitched', [leftPage[0].id, leftPage]); // 添加 pageSwitched 事件
        ...
        leftPage.trigger("pageAnimationStart", [leftPage[0].id, leftPage]); // light7的bug
        ...
        rightPage.trigger('pageSwitched', [rightPage[0].id, rightPage]);
        ...
        leftPage.trigger("pageInitInternal", [leftPage[0].id, leftPage]);
      }
    } else {
      if (!leftToRight) {
        rightPage.trigger("pageAnimationStart", [rightPage[0].id, rightPage]);
        ...
        leftPage.animationEnd(function() {
          ...
          leftPage.trigger('pageSwitched', [leftPage[0].id, leftPage]);
        });
  ...
  Router.prototype.go = function(steps, reload) {
    if ( !steps ) return;
    var backs = JSON.parse(this.stack.getItem("back")),
      forwords = JSON.parse(this.stack.getItem("forward")),
      lenBack = backs.length,
      lenForword = forwords.length,
      step = steps,
      currentPage = this.getCurrentPage();
    if ( steps < 0 ) {
      if ( lenBack == 0 ) return;
      else if ( -steps > lenBack ) step = -lenBack;
    } else if ( steps > 0 ) {
      if ( lenForword == 0 ) return;
      else if ( steps > lenForword ) step = lenForword;
    }

    if ( step > 1 ) {
      for ( var i=forwords.length-1; i>forwords.length-1-step; i-- ) {
        if ( i == forwords.length-1 ) {
          this.pushBack({url: location.href, pageid: "#"+currentPage[0].id, id: this.getCurrentStateID(), animation: forwords[i].animation, leftToRight: forwords[i].leftToRight});
        } else {
          this.pushBack({url: location.href, pageid: forwords[i+1].pageid, id: forwords[i+1].id, animation: forwords[i].animation, leftToRight: forwords[i].leftToRight});
          this.popForward();
        }
      }
      this.__go = true;
    } else if ( step < -1 ) {
      for ( var i=backs.length-1; i>backs.length-1+step; i-- ) {
        if ( i == backs.length-1 ) {
          this.pushForward({url: location.href, pageid: "#"+currentPage[0].id, id: this.getCurrentStateID(), animation: backs[i].animation, leftToRight: backs[i].leftToRight});
        } else {
          this.pushForward({url: location.href, pageid: backs[i+1].pageid, id: backs[i+1].id, animation: backs[i].animation, leftToRight: backs[i].leftToRight});
          this.popBack();
        }
      }
      this.__go = true;
  ...
  //如果无法前进，则加载对应的url
  Router.prototype.forward = function(url, reload) {
  ...
  //如果无法后退，则加载对应的url
  Router.prototype.back = function(url, reload) {
  ...
  Router.prototype._back = function() {
    ...
    if(!newPage[0]) return;
    if ( this.__go ) this.__go = false;
    else this.pushForward({url: location.href, pageid: "#"+currentPage[0].id, id: this.getCurrentStateID(), animation: h.animation, leftToRight: h.leftToRight});
    ...
    if ( this.__reload ) {
      ...
      this.loadPage({
        ...
        reload: true,
        isPushState: false,
        leftToRight: h.leftToRight != true,
        pageid: h.pageid.substring(1),
        isChangeState: false
      });
      ...
      currentPage.trigger('beforePageSwitch', [currentPage[0].id, currentPage]);
      // 添加回退事件
      newPage.trigger('o.back', [newPage[0].id, newPage]);
      if ( h.leftToRight )
        this.animatePages(currentPage, newPage, !h.leftToRight, !h.animation);
      else
        this.animatePages(newPage, currentPage, !h.leftToRight, !h.animation);
    }
  }
  ...
  Router.prototype._forward = function() {
    ...
    if(!newPage[0]) return;
    if ( this.__go ) this.__go = false;
    else this.pushBack({url: location.href, pageid: "#"+currentPage[0].id, id: this.getCurrentStateID(), animation: h.animation, leftToRight: h.leftToRight});
    this.setCurrentStateID(h.id);
    if ( this.__reload ) {
      this.__reload = false;
      this.loadPage({
      ...
      currentPage.trigger('beforePageSwitch', [currentPage[0].id, currentPage]);
      // 添加前进事件
      newPage.trigger('o.forward', [newPage[0].id, newPage]);
      if ( h.leftToRight )
        this.animatePages(newPage, currentPage, h.leftToRight, !h.animation);
      else
        this.animatePages(currentPage, newPage, h.leftToRight, !h.animation);
  ...
  Router.prototype.pushState = function(url, id, leftToRight) {
    history.pushState({url: url, id: id, leftToRight: leftToRight}, '', url);
  }

  Router.prototype.replaceState = function(url, id, leftToRight) {
    history.replaceState({url: url, id: id, leftToRight: leftToRight}, '', url);
  }
  ...
  Router.prototype.getPage = function(url, callback, pageid) {
    ...
    $(window).trigger( "beforeload_ajax", [self, url] );	// copy上一版 lixx: 新加一个事件，在真正调用ajax加载页面前
    this.xhr = $.ajax({
      url: url,
      success: $.proxy(function(data, s, xhr) {
        ...
        var $page = html[0];
        var $extra = html[1];
        if ( pageid && !$page[0].id ) $page[0].id = pageid;
        else if(!$page[0].id) $page[0].id = this.genRandomID();
        $page.data("page-remote", 1).data('url', url).attr('data-url', encodeURIComponent(url)); // 记录url
        $('body').trigger('pageLoadSuccess', [$page, $extra]); // 添加pageLoadSuccess完成事件
  ...
  $(function() {
    ...
    $(document).on("click", "a", function(e) {
      ...
      if($target.hasClass("back")) {
        router.back(url, $target.data('reload'));
        return;
      }
